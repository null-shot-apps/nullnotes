  ...
  const convertVideo = async () => {
    ...
    try {
      ...
      const qualitySettings = {
        low: outputFormat === 'gif' ? { fps: 10, scale: 320 } : { quality: 60, fps: 10, scale: 320 },
        medium: outputFormat === 'gif' ? { fps: 15, scale: 480 } : { quality: 75, fps: 15, scale: 480 },
        high: outputFormat === 'gif' ? { fps: 24, scale: 720 } : { quality: 90, fps: 24, scale: 720 },
      };
      ...
      const data = await ffmpeg.readFile(outputFormat === 'gif' ? 'output.gif' : 'output.webp');
      const blob = new Blob([data], { type: outputFormat === 'gif' ? 'image/gif' : 'image/webp' });
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
              <label className="block">
                <div className="border-2 border-dashed border-white/40 rounded-xl p-8 text-center cursor-pointer hover:border-white/60 transition">
                  <input
                    type="file"
                    accept="video/*"
                  ...
                  <p className="text-lg font-medium">
                    {videoFile ? videoFile.name : 'Загрузи видео'}
                  </p>
                ...
                {videoDuration > 0 && (
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-white/70">
                      Длительность фрагмента: {formatTime(endTime - startTime)}
                    </p>
                  </div>
                )}
              </div>
            ...
            {videoFile && (
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 space-y-4">
                ...
                <button
                  ...
                  disabled={converting}
                  className="w-full py-4 px-6 bg-gradient-to-r from-pink-500 to-orange-500 rounded-lg font-bold text-lg hover:from-pink-600 hover:to-orange-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {converting ? `Конвертируем... ${progress}%` : 'Конвертировать'}
                </button>
              </div>
            )}
            ...
            {outputUrl && (
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
                ...
                <button
                  ...
                  className="w-full py-4 px-6 bg-green-500 rounded-lg font-bold text-lg hover:bg-green-600 transition"
                >
